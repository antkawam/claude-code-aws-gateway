...
fn is_allowed_cors_origin(origin: &HeaderValue) -> bool {
    origin
        .to_str()
        .map(|s| s == "https://claude.ai" || (s.ends_with(".claude.ai") && s.starts_with("https://")))
        .unwrap_or(false)
}
...
mod tests {
    ...
    fn test_cors_allows_claude_ai() {
        assert!(is_allowed_cors_origin(&HeaderValue::from_static("https://claude.ai")));
    }

    #[test]
    fn test_cors_allows_subdomain() {
        assert!(is_allowed_cors_origin(&HeaderValue::from_static("https://pivot.claude.ai")));
        assert!(is_allowed_cors_origin(&HeaderValue::from_static("https://excel.claude.ai")));
        assert!(is_allowed_cors_origin(&HeaderValue::from_static("https://anything.claude.ai")));
    }

    #[test]
    fn test_cors_blocks_unrelated_origins() {
        assert!(!is_allowed_cors_origin(&HeaderValue::from_static("https://evil.com")));
        assert!(!is_allowed_cors_origin(&HeaderValue::from_static("https://example.com")));
    }

    #[test]
    fn test_cors_blocks_http() {
        assert!(!is_allowed_cors_origin(&HeaderValue::from_static("http://claude.ai")));
        assert!(!is_allowed_cors_origin(&HeaderValue::from_static("http://pivot.claude.ai")));
    }

    #[test]
    fn test_cors_blocks_suffix_spoofing() {
        assert!(!is_allowed_cors_origin(&HeaderValue::from_static("https://fakeclaude.ai")));
        assert!(!is_allowed_cors_origin(&HeaderValue::from_static("https://notclaude.ai")));
        assert!(!is_allowed_cors_origin(&HeaderValue::from_static("https://evil.claude.ai.attacker.com")));
    }
}

#!/bin/bash
# Build, push, and deploy via CDK (default) or direct ECS update (--fast).
#
# Default path: build ARM64 image -> push to ECR -> cdk deploy (handles code + infra).
...
#   .claude/scripts/deploy.sh --prod                 # Build + CDK deploy (production)
#   .claude/scripts/deploy.sh --staging --fast       # Build + direct ECS update (code-only)
#   .claude/scripts/deploy.sh --staging --skip-build # Re-deploy last built image via CDK
...
ECR_REPO_NAME=$(jq -r ".ecr_repo_name" "$ENV_FILE")
MIN_TASKS=$(jq -r ".${ENVIRONMENT}.desired_count" "$ENV_FILE")
ECR_REPO="${EXPECTED_ACCOUNT}.dkr.ecr.${REGION}.amazonaws.com/${ECR_REPO_NAME}"
...
if [ -n "$PRE_BUILT_IMAGE" ]; then
    ...
    if ! docker image inspect "$IMAGE" &>/dev/null; then
        if docker image inspect "${ECR_REPO}:latest" &>/dev/null; then
            echo ""
            echo "==> Re-tagging latest -> ${TAG}"
            docker tag "${ECR_REPO}:latest" "$IMAGE"
        else
            echo "    ERROR: No local image found for ${TAG} or latest."
...
# Push to ECR (needed for both CDK and ECS paths)
if [ -z "$PRE_BUILT_IMAGE" ]; then
    echo ""
    echo "==> Pushing to ECR..."
    aws ecr get-login-password --region "$REGION" | \
        docker login --username AWS --password-stdin "${EXPECTED_ACCOUNT}.dkr.ecr.${REGION}.amazonaws.com" 2>/dev/null
    docker push "$IMAGE" 2>&1 | grep -E "digest:|Pushed|existed" | tail -3 || true
    echo "    Pushed ${TAG}"
fi

# ---- Resolve image digest (for CDK content-based change detection) ----
...
if [ -n "$IMAGE_DIGEST" ] && [ "$IMAGE_DIGEST" != "None" ]; then
    ...
    echo "    WARN: Could not resolve digest, falling back to tag"
    IMAGE_DIGEST=""
fi
...
if [ "$FAST" = true ]; then
    ...
    if [ -n "$IMAGE_DIGEST" ]; then
        CDK_ARGS+=(-c "imageDigest=${IMAGE_DIGEST}")
    fi

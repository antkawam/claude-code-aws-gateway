...
.claude/scripts/deploy.sh --prod                 # Production (builds + pushes same flow)
.claude/scripts/deploy.sh --prod --skip-build    # Reuse existing GHCR image (same tag as staging)
...
- Images are built locally (ARM64) and pushed to GHCR (`ghcr.io/<owner>/<repo>:<commit>`)
- CDK pulls from GHCR by default (no ECR repo needed)
- Authenticates via AWS profile (credential_process in `~/.aws/config`, refreshed by isengardcli)
- GHCR auth via GitHub CLI (`gh auth token`) — requires `gh auth login` once
...
4. `.claude/scripts/deploy.sh --staging` → build, push GHCR, deploy staging
5. `.claude/scripts/deploy.sh --prod --skip-build` → same image to prod
6. `.claude/scripts/deploy.sh --prod --release` → cut GitHub release (tags repo, GH Actions re-tags GHCR with semver)
